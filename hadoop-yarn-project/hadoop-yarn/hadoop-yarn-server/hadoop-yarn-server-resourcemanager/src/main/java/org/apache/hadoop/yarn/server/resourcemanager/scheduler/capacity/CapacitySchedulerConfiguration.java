...
public class CapacitySchedulerConfiguration extends ReservationSchedulerConfiguration {
  ...
  public Map<String, QueueCapacityVector> parseConfiguredResourceVector(
    ...
    for (String label : labels) {
      String propertyName = QueuePrefixes.getNodeLabelPrefix(
          queuePath, label) + CAPACITY;
      String capacityString = get(propertyName);
      queueResourceVectors.put(label,
          queueCapacityConfigParser.parse(capacityString, queuePath));
    }
  ...
  public Map<String, QueueCapacityVector> parseConfiguredMaximumCapacityVector(
    ...
    for (String label : labels) {
      String propertyName = QueuePrefixes.getNodeLabelPrefix(
          queuePath, label) + MAXIMUM_CAPACITY;
      String capacityString = get(propertyName);
      QueueCapacityVector capacityVector = queueCapacityConfigParser.parse(
          capacityString, queuePath);
  ...
  public boolean checkConfigTypeIsAbsoluteResource(String label,
      QueuePath queue, Set<String> resourceTypes) {
    return checkConfigTypeIsAbsoluteResource(label, queue);
  }
