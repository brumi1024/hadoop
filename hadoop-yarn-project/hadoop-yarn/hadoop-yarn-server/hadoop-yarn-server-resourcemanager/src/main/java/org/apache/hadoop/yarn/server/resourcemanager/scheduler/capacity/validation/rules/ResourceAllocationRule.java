...
abstract class ResourceAllocationRule implements ValidationRule {
  ...
  ResourceAllocationRule(String ruleId, String minimumKey, String maximumKey,
  ...
  static void validate(Configuration conf, String minimumKey,
      String maximumKey, int defaultMinimum, int defaultMaximum,
      String resourceNoun) {
    int min = conf.getInt(minimumKey, defaultMinimum);
    int max = conf.getInt(maximumKey, defaultMaximum);
    if (min <= 0 || min > max) {
      throw new YarnRuntimeException(errorMessage(resourceNoun));
    }
  }
